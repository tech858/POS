<div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='request-{{ $request->id . rand(1111, 9999) . microtime() }}' wire:loading.class.delay='opacity-10'>
                            ...
                            <td class="py-2.5 px-4 space-x-2 whitespace-nowrap text-right dark:text-white">
                                <x-dropdown align="right" dropdownClasses="z-10">
                                    <x-slot name="trigger">
                                    ...
                                    <x-slot name="content">
                                        <!-- Dropdown menu -->
                                        <x-dropdown-link wire:click="ViewRequest('{{ $request->id }}')" class="block px-4 py-2 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            @lang('app.view')
                                        </x-dropdown-link>
                                        <x-dropdown-link wire:click="downloadFile({{ $request->id }})" class="block px-4 py-2 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                            @lang('app.download')
                                        </x-dropdown-link>
                                        @if ($request->status == 'pending')
                                            <x-dropdown-link wire:click="confirmChangePlan({{ $request->id }}, 'verified')" class="block px-4 py-2 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                @lang('app.accept')
                                            </x-dropdown-link>
                                            <x-dropdown-link wire:click="confirmChangePlan({{ $request->id }}, 'rejected')" class="block px-4 py-2 cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                @lang('app.decline')
                                            </x-dropdown-link>
                                        @endif
                                    </x-slot>
    ...
    <x-dialog-modal wire:model="showConfirmChangeModal">
        <x-slot name="title">
            @if ($status == 'verified')
            @lang('app.acceptRequest')
            @elseif ($status == 'rejected')
            @lang('app.declineRequest')
            @endif
        </x-slot>
