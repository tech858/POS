...
class SendOrderBill extends BaseNotification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return $build
            ->subject(__('email.sendOrderBill.subject'). ' #'.$this->order->order_number)
            ->markdown('emails.order-bill', [
                        'order' => $this->order,
                        'subtotal' => number_format($this->order->sub_total, 2),
                        'taxesWithAmount' => $taxesWithAmount,
                        'totalPrice' => number_format($this->order->total, 2),
    ...
    protected function formatOrderSummary($items)
    {
        return $items->map(function ($item) {
            return $item->quantity . ' x ' . $item->name . ' @ $' . number_format($item->price, 2);
        })->implode(', ');
    }
